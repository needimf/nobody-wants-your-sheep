      ...
      <React.Fragment>
        <div className='container-fluid' id='gameSidebar'>
          <div className='playerBar'>
            <h2>Pick a color</h2>
            {this.state.selected ? <div> You are player {this.state.number} <br></br> <button type='button' onClick={(e) => { e.stopPropagation(); this.props.deselectColor(this.state.color, this.state.number); this.setState({ ...this.state, selected: false }) }}>Deselect Color</button> <br></br> <button type='button' onClick={(e) => { e.stopPropagation(); this.props.start(this.state.number, this.state.color, this.state.color.toLowerCase()) }}>Keep Going?</button>
